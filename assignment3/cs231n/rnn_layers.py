...
def rnn_step_backward(dnext_h, cache):
    ...
    db = ((1 - np.tanh(u) ** 2) * dnext_h).sum(axis = 0).flatten()
    dWh = (prev_h.T).dot(dnext_h * (1 - np.tanh(u) ** 2))
    dWx = x.T.dot(dnext_h * (1 - np.tanh(u) ** 2))
    dprev_h = (dnext_h * (1 - np.tanh(u) ** 2)).dot(Wh.T)
    dx = (dnext_h * (1 - np.tanh(u) ** 2)).dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        current_h, catched = rnn_step_forward(x[:, t , :], current_h, Wx, Wh, b)
        h[:, t, :] = current_h
        cache += [catched]
...
def rnn_backward(dh, cache):
    ...
    for i in Ts:
        ...
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_current, cache[i])
        dx[:, i, :] += dx_t
        dh0 = dprev_h
        dWx += dWx_t
        dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i_t, f_t, o_t = sigmoid(a_i), sigmoid(a_f), sigmoid(a_o)
    g_t = np.tanh(a_g)

    next_c = f_t * prev_c + i_t * g_t
    next_h = o_t * np.tanh(next_c)
    cache = (f_t, i_t, o_t, g_t, prev_h, prev_c, next_c, next_h, x, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    f, i, o, g, prev_h, prev_c, next_c, next_h, x, Wx, Wh = cache
    alt_dnext_c = dnext_h * o * (1 - np.tanh(next_c)**2)
    dprev_c = dnext_c * f + alt_dnext_c * f
    d_f = dnext_c * prev_c + alt_dnext_c * prev_c
    d_i = alt_dnext_c * g + dnext_c * g
    d_o = dnext_h * np.tanh(next_c)
    d_g = alt_dnext_c * i + dnext_c * i
    i_a = d_i * i * (1 - i)
    f_a = d_f * f * (1-f)
    o_a = d_o * o * (1-o)
    g_a = d_g * (1 - g **2)
    d_a = np.hstack((i_a, f_a, o_a, g_a))
...
def lstm_backward(dh, cache):
    ...
    f_t, i_t, o_t, g_t, prev_h, prev_c, next_c, next_h, x, Wx, Wh = cache[0]
    N, D = x.shape
    T = len(cache)
    H = next_h.shape[1]
    ...
    for i in range(T):
        i = T - 1 - i
        dprev_h += dh[:, i, :]
        dx[:, i, :], dprev_h, dprev_c, dWx_live, dWh_live, db_live = lstm_step_backward(dprev_h, dprev_c, cache[i])
        dWx += dWx_live
        dWh += dWh_live
        db  += db_live
    dh0 = dprev_h
