...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        loss, dx_scores = temporal_softmax_loss(out, captions_out, mask, verbose=False)
        dh, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dx_scores, out_cache)
        if self.cell_type == "rnn":
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(1, max_length):
            x, _ = word_embedding_forward(captions[:, t -1], W_embed)
            if self.cell_type == "rnn":
