...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.num_heads
        Q = torch.permute(torch.reshape(self.query(query), (N, S, H, D//H)), (0, 2, 1, 3))
        K = torch.permute(torch.reshape(self.key(key), (N, T, H, D//H)), (0, 2, 3, 1))
        V = torch.permute(torch.reshape(self.value(value), (N, T, H, D//H)), (0, 2, 1, 3))
        ...
        QK = self.dropout(QK)
        QKV = torch.reshape(torch.permute(torch.matmul(QK, V), (0, 2, 1, 3)), (N, S, D))
        output = self.proj(QKV)
